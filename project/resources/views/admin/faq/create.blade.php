@extends('layouts.app', ['activePage' => 'faq-management', 'titlePage' => __('FAQ Management')])
...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{ route('faq-create') }}" autocomplete="off" class="form-horizontal">
            @csrf
            @method('post')

            <div class="card ">
              <div class="card-header card-header-primary">
                <h4 class="card-title">{{ __('Add FAQ Question') }}</h4>
                <p class="card-category"></p>
              </div>
              <div class="card-body ">
                <div class="row">
                  <div class="col-md-12 text-right">
                      <a href="{{ route('faq-management') }}" class="btn btn-sm btn-primary">{{ __('Back to list') }}</a>
                  </div>
                </div>
                <div class="row">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('alias') ? ' has-danger' : '' }}">
                        <input class="form-control{{ $errors->has('alias') ? ' is-invalid' : '' }}" style="width: 90%; float: left;" name="alias" id="input-alias" type="text" placeholder="{{ __('Alias') }}" value="{{ old('alias') }}" required="true" aria-required="true"/>
                        <img src="/material/img/lock_off22.png"
                             style="padding-top: 10px; cursor:pointer; float: right;" onclick="lockLabelInput(this);"/>
                        <input type="hidden" value="0" name="lockalias" id="lock_alias_field"/>
                      @if ($errors->has('alias'))
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('category') ? ' has-danger' : '' }}">
                        <select class="form-control custom-select" data-style="btn btn-link" id="category-select" name="category">
                            @foreach($categories as $category)
                                <option value="{{ $category->getIdentity() }}">{{ $category->getTitle() }}</option>
                            @endforeach
                        </select>
                    </div>
                  </div>
                  ...
                  <div class="row">
                      ...
                      <div class="col-sm-7">
                          <div class="form-group{{ $errors->has('answer') ? ' has-danger' : '' }}">
                              <textarea id="editor" name="answer" rows="5"></textarea>
                          </div>
                      </div>
                  </div>
              </div>

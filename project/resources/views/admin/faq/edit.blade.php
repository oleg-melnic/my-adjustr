...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{ route('faq-update') }}" autocomplete="off" class="form-horizontal">
            @csrf
            @method('put')
              <input type="hidden" name="questionId" value="{{ $item->getIdentity() }}">

            <div class="card ">
              <div class="card-header card-header-primary">
                <h4 class="card-title">{{ __('Edit FAQ Question') }}</h4>
                          ...
                          <div class="form-group{{ $errors->has('category') ? ' has-danger' : '' }}">
                              <select class="form-control custom-select" data-style="btn btn-link" id="category-select" name="category">
                                  @foreach($categories as $category)
                                      <option value="{{ $category->getIdentity() }}" @if($category->getIdentity() === $item->getCategory()->getIdentity())selected @endif>{{ $category->getTitle() }}</option>
                                  @endforeach
                              </select>
                          </div>
                      </div>
                    ...
                    <label class="col-sm-2 col-form-label">{{ __('Answer') }}</label>
                      <div class="col-sm-7">
                        <div class="form-group{{ $errors->has('answer') ? ' has-danger' : '' }}">
                            <textarea id="editor" name="answer" style="min-height: 500px;">{{ $item->getAnswer() }}</textarea>
                        </div>
                      </div>
                  </div>
              </div>
